...
class SnowflakeCredentials:
    account: str = config("account", default="ldyglyl-tba85072")
    user: str = config("user", default="FRANCOALARCON00")
    password: str = config("password", default="Samikity12")
    database: str = config("database", default="SODAREPORTING")
    schema: str = config("schema", default="PUBLIC")
    warehouse: str = config("warehouse", default="SODAREPORTING")
...
def push_to_db(
    ...
    # We don't use sql format to export the df, but a csv we later upload on snowflake cloud
